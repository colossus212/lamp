...
static uint16_t frq = 0;//1-1000:(0.1-100Hz)
...
#endif
void trig_mode(uint8_t mode, uint16_t nt)//num of times
{
	switch (mode)
	{
		case 0://单次触发
...
#ifdef FINSH_USING_SYMTAB
FINSH_FUNCTION_EXPORT(trig_mode, trig_mode single 0 mult 1 serial 2);
#endif
